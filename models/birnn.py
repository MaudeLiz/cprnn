...
class BiRNN(nn.Module):
    """BiRNN. Outputs logits (no softmax)
    ...
    def __init__(self, input_size: int, hidden_size: int, vocab_size: int, use_embedding: bool = False,
        ...
        self.w = nn.Parameter(torch.Tensor(self.hidden_size, self.input_size, self.hidden_size))
        self.init_weights()
    ...
    def forward(self, inp: torch.LongTensor, init_states: torch.Tensor = None):
        ...
        for t in range(sequence_length):
            ...
            h_t = self.gate(torch.einsum("bi,bj,ijk->bk", h_t, x_t, self.w))
